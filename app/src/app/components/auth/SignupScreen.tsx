...
const RegisterScreen = () => {
  ...
  const handleRegister = async () => {
    if (!name || !email || !password || !confirmPassword || !verificationCode || !institutionId) {
      showToast('error', 'Erro de Cadastro', 'Por favor, preencha todos os campos, incluindo instituição e código de verificação.');
      return;
    }
...
>>>>>>> f6757a7 (``` ✅ Fix form validation to require institution selection during signup  This commit improves the signup form validation by making the institution field mandatory.  The validation check now explicitly requires institutionId to be filled, and the error  message has been updated to clearly indicate that both institution and verification code  are required fields. Also fixed indentation of the showToast function for consistency. ```)
